namespace ExampleFramework.NodeEditorZoom
{
    ...
    using Hexa.NET.ImNodesZoom;
    using System.Collections.Generic;

    public class NodeEditorZoom
    {
        ...
        public virtual void Initialize()
        {
            if (context.IsNull)
            {
                context = ImNodesZoom.EditorContextCreate();
        ...
        {
            return ImNodesZoom.SaveEditorStateToIniStringS(context, null);
        }
        ...
        public void RestoreState(string state)
        {
            ...
            ImNodesZoom.LoadEditorStateFromIniString(context, state, (uint)state.Length);
        }
        ...
        public void Draw()
        {
            if (context.IsNull)
                Initialize();
            ImNodesZoom.EditorContextSet(context);
            ImNodesZoom.BeginNodeEditor();
            for (int i = 0; i < Nodes.Count; i++)
            {
                Nodes[i].Draw();
            }
            ...
            if (ImNodesZoom.IsLinkCreated(ref idNode1, ref idpin1, ref idNode2, ref idpin2, ref createdFromSnap))
            {
            ...
            int idLink = 0;
            if (ImNodesZoom.IsLinkDestroyed(ref idLink))
            {
                GetLink(idLink).Destroy();
            }
            if (ImGuiP.IsKeyPressed(ImGuiKey.Delete))
            {
                int numLinks = ImNodesZoom.NumSelectedLinks();
                if (numLinks != 0)
                {
                    int[] links = new int[numLinks];
                    ImNodesZoom.GetSelectedLinks(ref links[0]);
                    for (int i = 0; i < links.Length; i++)
                    {
                        GetLink(links[i]).Destroy();
                    }
                }
                int numNodes = ImNodesZoom.NumSelectedNodes();
                if (numNodes != 0)
                {
                    int[] nodes = new int[numNodes];
                    ImNodesZoom.GetSelectedNodes(ref nodes[0]);
            ...
            int idpinStart = 0;
            if (ImNodesZoom.IsLinkStarted(ref idpinStart))
            {
            }
            ...
            {
                var id = Nodes[i].Id;
                Nodes[i].IsHovered = ImNodesZoom.IsNodeHovered(ref id);
            }

            if (ImNodesZoom.IsEditorHovered() && ImGui.GetIO().MouseWheel != 0)
            {
                float zoom = ImNodesZoom.EditorContextGetZoom() + ImGui.GetIO().MouseWheel * 0.1f;
                ImNodesZoom.EditorContextSetZoom(zoom, ImGui.GetMousePos());
            }

            ImNodesZoom.EditorContextSet(null);
        ...
        public void Destroy()
        {
            ...
            this.nodes.Clear();
            ImNodesZoom.EditorContextFree(context);
            context = null;
        }
