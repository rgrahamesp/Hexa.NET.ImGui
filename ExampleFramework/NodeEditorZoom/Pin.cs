...
#pragma warning restore CS8618 // Non-nullable field 'parent' must contain a non-null value when exiting constructor. Consider declaring the field as nullable.
        ...
        public void Draw()
        {
            ...
            {
                ImNodesZoom.BeginInputAttribute(id, Shape);
                DrawContent();
                ImNodesZoom.EndInputAttribute();
            }
            if (Kind == PinKind.Output)
            {
                ImNodesZoom.BeginOutputAttribute(id, Shape);
                DrawContent();
                ImNodesZoom.EndOutputAttribute();
            }
            if (Kind == PinKind.Static)
            {
                ImNodesZoom.BeginStaticAttribute(id);
                DrawContent();
                ImNodesZoom.EndStaticAttribute();
            }
        }
        ...
        public virtual void Initialize(NodeEditorZoom editor, Node parent)
        {
            this.editor = editor;
            this.parent = parent;
            if (id == 0)
