namespace ExampleFramework.NodeEditorZoom
{
    ...
    public class Node
    {
        ...
        public virtual void Initialize(NodeEditorZoom editor)
        {
            this.editor = editor;
            if (id == 0)
        ...
        public virtual Pin CreatePin(NodeEditorZoom editor, string name, PinKind kind, PinType type, ImNodesPinShape shape, uint maxLinks = uint.MaxValue)
        {
            Pin pin = new(editor.GetUniqueId(), name, shape, kind, type, maxLinks);
            return AddPin(pin);
        }

        public virtual Pin CreateOrGetPin(NodeEditorZoom editor, string name, PinKind kind, PinType type, ImNodesPinShape shape, uint maxLinks = uint.MaxValue)
        {
            Pin pin = new(editor.GetUniqueId(), name, shape, kind, type, maxLinks);
        ...
        {
            ImNodesZoom.PushColorStyle(ImNodesCol.TitleBar, TitleColor);
            ImNodesZoom.PushColorStyle(ImNodesCol.TitleBarHovered, TitleHoveredColor);
            ImNodesZoom.PushColorStyle(ImNodesCol.TitleBarSelected, TitleSelectedColor);
            ImNodesZoom.BeginNode(id);
            ImNodesZoom.BeginNodeTitleBar();
            if (isEditing)
            {
                string name = Name;
